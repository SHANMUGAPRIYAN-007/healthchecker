...
const PatientDetails = () => {
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            try {
                // In a real app, you'd have a specific endpoint for doctor to get patient profile
                // For now, let's fetch records and infer name (simulated)
                const res = await axios.get('/api/records');
                // Filtering current patient records (simulating doctor access)
                const patientRecords = res.data.filter(r => r.patientId === id);
                setRecords(patientRecords);
                setPatient({ name: 'Patient Client', id }); // Placeholder name
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
